...
function DocList({ title, docs, onDecide, onDownload }) {
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
        {docs.map((d) => (
          <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", padding: "10px 14px", border: "1px solid var(--a-border)", borderRadius: 10 }}>
            ...
            <button type="button" className="amx-btn amx-btn-outline amx-btn-sm" onClick={() => onDownload(d.id, d.fileName)}>{d.fileName}</button>
            {d.documentNumber && <span className="amx-cell-sub">#{d.documentNumber}</span>}
            <StatusBadge status={d.status === "approved" ? "active" : d.status === "rejected" ? "inactive" : "pending"} label={d.status.replaceAll("_", " ")} />
            {d.reviewerRemarks && <span className="amx-cell-sub" style={{ fontStyle: "italic" }}>"{d.reviewerRemarks}"</span>}
            <div style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
              <button type="button" className="amx-btn amx-btn-outline amx-btn-sm" onClick={() => onDecide(d.id, "approved")}>Approve</button>
              <button type="button" className="amx-btn amx-btn-outline amx-btn-sm" onClick={() => onDecide(d.id, "rejected")}>Reject</button>
              <button type="button" className="amx-btn amx-btn-outline amx-btn-sm" onClick={() => onDecide(d.id, "replacement_requested")}>Request Replacement</button>
            </div>
          </div>
        ))}
      </div>
    ...
    <>
      ...
      <div className="amx-card amx-panel" style={{ marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 20 }}>
          <DocList title="Identity Documents" docs={repDocs} onDecide={decideDocument} onDownload={downloadDocument} />
        </div>
      </div>

      <div className="amx-card amx-panel" style={{ marginBottom: 20 }}>
        <DocList title="Masjid Documents" docs={masjidDocs} onDecide={decideDocument} onDownload={downloadDocument} />
        <DocList title="Property Verification" docs={propertyDocs} onDecide={decideDocument} onDownload={downloadDocument} />
      </div>
