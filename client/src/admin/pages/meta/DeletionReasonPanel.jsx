...
function ReasonForm({ reason, onCancel, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = isEdit
        ? await adminApi.patch(`/deletion-reasons/${reason.id}`, { name: name.trim(), isActive })
        : await adminApi.post("/deletion-reasons", { name: name.trim(), isActive });
    ...
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>
          {inUse
            ? `${reason.usageCount} past deletion${reason.usageCount === 1 ? "" : "s"} already used this reason and will keep it on record, but it will no longer be offered on the Delete Masjid form.`
            : "This reason will no longer be offered on the Delete Masjid form."}
        </p>
...
function DeletionReasonPanel() {
  ...
  const activate = async (reason) => {
    ...
    try {
      await adminApi.patch(`/deletion-reasons/${reason.id}`, { isActive: true });
  ...
  const confirmDeactivate = async () => {
    ...
    setBusyId(reason.id);
    try {
      await adminApi.patch(`/deletion-reasons/${reason.id}`, { isActive: false });
  ...
  const confirmDelete = async () => {
    ...
    setBusyId(reason.id);
    try {
      await adminApi.delete(`/deletion-reasons/${reason.id}`);
  ...
  return (
    <>
      <div className="amx-panel-head">
        <div>
          <h3>Masjid Deletion Reasons</h3>
          <div className="amx-panel-sub">Master list of reasons offered when a user deletes a masjid from their account</div>
        </div>
...
export default DeletionReasonPanel;
