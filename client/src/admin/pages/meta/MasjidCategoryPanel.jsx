...
const SORT_COLUMNS = {
  ...
  masjidCount: { label: "Registered Masjids", get: (c) => c.masjidCount || 0 },
...
function CategoryFormModal({ category, onCancel, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = isEdit
        ? await adminApi.patch(`/masjid-categories/${category.id}`, { name: name.trim(), isActive })
        : await adminApi.post("/masjid-categories", { name: name.trim(), isActive });
...
function ConfirmDeactivateModal({ category, onCancel, onConfirm, busy }) {
  const inUse = (category.masjidCount || 0) > 0;
  return (
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        <button className="amx-modal-close" onClick={onCancel} aria-label="Close"><Icon name="x" size={16} /></button>
        ...
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>
          {inUse
            ? `${category.masjidCount} masjid${category.masjidCount === 1 ? "" : "s"} currently registered under this category will keep it, but it will no longer be offered for new masjid registrations.`
            : "This category will no longer be available for new masjid registrations."}
        </p>
...
function MasjidCategoryPanel() {
  ...
  const load = () => {
    ...
    adminApi
      ...
      .catch((err) => setError(err.response?.data?.message || "Couldn't load masjid categories."))
  ...
  const activate = async (category) => {
    ...
    try {
      await adminApi.patch(`/masjid-categories/${category.id}`, { isActive: true });
  ...
  const confirmDeactivate = async () => {
    ...
    setBusyId(category.id);
    try {
      await adminApi.patch(`/masjid-categories/${category.id}`, { isActive: false });
  ...
  return (
    <>
      <div className="amx-panel-head">
        <div>
          <h3>Masjid Category</h3>
          <div className="amx-panel-sub">Master list of categories offered when registering a masjid</div>
        </div>
        <button className="amx-btn amx-btn-primary" onClick={() => setFormModal("new")}>
          <Icon name="plus" size={15} /> Add Masjid Category
        </button>
      </div>
      ...
      {loading ? (
        <div className="amx-empty">
          <Icon name="mosque" />
          <strong>Loading masjid categories…</strong>
        </div>
      ) : filtered.length === 0 ? (
        <div className="amx-empty">
          <Icon name="mosque" />
          ...
          <table className="amx-table">
            <thead>
              <tr>
                ...
                <SortHeader label="Registered Masjids" sortKey="masjidCount" activeKey={sortKey} direction={sortDir} onSort={toggleSort} />
...
export default MasjidCategoryPanel;
