...
const SORT_COLUMNS = {
  name: { label: "Degree / Qualification", get: (s) => s.name?.toLowerCase() || "" },
...
function DegreeForm({ degree, onCancel, onSaved }) {
  const isEdit = !!degree;
  const [name, setName] = useState(degree?.name || "");
  const [isActive, setIsActive] = useState(degree ? degree.isActive : true);
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = isEdit
        ? await adminApi.patch(`/degrees/${degree.id}`, { name: name.trim(), isActive })
        : await adminApi.post("/degrees", { name: name.trim(), isActive });
      onSaved(data.degree, isEdit);
    } catch (err) {
      setError(err.response?.data?.message || "Couldn't save this degree.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <button className="amx-back-link" onClick={onCancel}>
        <Icon name="arrowRight" size={14} style={{ transform: "rotate(180deg)" }} /> Back to Degree / Qualification
      </button>
      <h3 style={{ marginBottom: 20 }}>{isEdit ? "Edit Degree" : "Add Degree"}</h3>
      <form onSubmit={submit} style={{ maxWidth: 480 }}>
        <div className="amx-form-group">
          <label htmlFor="degree-name">Degree / Qualification Name</label>
          <input id="degree-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. B.Tech." autoFocus maxLength={255} />
          {error && (
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button type="submit" className="amx-btn amx-btn-primary" disabled={saving || !name.trim()}>
            {saving ? "Saving…" : isEdit ? "Save Changes" : "Add Degree"}
          </button>
...
function ConfirmDeactivateModal({ degree, onCancel, onConfirm, busy }) {
  const inUse = (degree.usageCount || 0) > 0;
  return (
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 style={{ textAlign: "center" }}>Deactivate "{degree.name}"?</h3>
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>
          {inUse
            ? `${degree.usageCount} member${degree.usageCount === 1 ? "" : "s"} already have this degree and will keep it, but it will no longer be offered in the profile picker.`
            : "This degree will no longer be offered in the profile picker."}
        </p>
...
function ConfirmDeleteModal({ degree, onCancel, onConfirm, busy }) {
  return (
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        <button className="amx-modal-close" onClick={onCancel} aria-label="Close"><Icon name="x" size={16} /></button>
        <div className="amx-modal-danger-icon"><Icon name="trash" size={22} /></div>
        <h3 style={{ textAlign: "center" }}>Delete "{degree.name}"?</h3>
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>This can't be undone. Members who already used this degree keep it on their profile either way.</p>
        <div style={{ display: "flex", gap: 10 }}>
          <button className="amx-btn amx-btn-outline" style={{ flex: 1 }} onClick={onCancel} disabled={busy}>Cancel</button>
...
function DegreePanel() {
  ...
  const load = () => {
    ...
    adminApi
      ...
      .catch((err) => setError(err.response?.data?.message || "Couldn't load degrees."))
  ...
  const upsertDegree = (degree, isEdit) => {
    setDegrees((ss) => (isEdit ? ss.map((s) => (s.id === degree.id ? degree : s)) : [...ss, degree]));
    setFormModal(null);
    showToast(isEdit ? "Degree updated." : "Degree added.");
  };

  const activate = async (degree) => {
    setBusyId(degree.id);
    const prev = degree.isActive;
    setDegrees((ss) => ss.map((s) => (s.id === degree.id ? { ...s, isActive: true } : s)));
    try {
      await adminApi.patch(`/degrees/${degree.id}`, { isActive: true });
      showToast("Degree activated.");
    } catch (err) {
      setDegrees((ss) => ss.map((s) => (s.id === degree.id ? { ...s, isActive: prev } : s)));
      showToast(err.response?.data?.message || "Couldn't activate this degree.");
    } finally {
      setBusyId(null);
    }
  };
  ...
  const confirmDeactivate = async () => {
    const degree = deactivating;
    setBusyId(degree.id);
    try {
      await adminApi.patch(`/degrees/${degree.id}`, { isActive: false });
      setDegrees((ss) => ss.map((s) => (s.id === degree.id ? { ...s, isActive: false } : s)));
      showToast("Degree deactivated.");
    } catch (err) {
      showToast(err.response?.data?.message || "Couldn't deactivate this degree.");
    } finally {
      setBusyId(null);
  ...
  const confirmDelete = async () => {
    const degree = deleting;
    setBusyId(degree.id);
    try {
      await adminApi.delete(`/degrees/${degree.id}`);
      setDegrees((ss) => ss.filter((s) => s.id !== degree.id));
      showToast("Degree deleted.");
    } catch (err) {
      showToast(err.response?.data?.message || "Couldn't delete this degree.");
    } finally {
      setBusyId(null);
      setDeleting(null);
    }
  ...
  if (formModal) {
    return <DegreeForm degree={formModal === "new" ? null : formModal} onCancel={() => setFormModal(null)} onSaved={upsertDegree} />;
  }
    ...
    <>
      <div className="amx-panel-head">
        <div>
          ...
          <div className="amx-panel-sub">Master list of degrees offered in the Education section of a profile</div>
        </div>
        <button className="amx-btn amx-btn-primary" onClick={() => setFormModal("new")}>
          <Icon name="plus" size={15} /> Add Degree
        </button>
      </div>
      ...
      <div className="amx-filters">
        <div className="amx-search">
          <Icon name="search" />
          <input type="text" placeholder="Search by degree name…" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
      ...
      {loading ? (
        <div className="amx-empty">
          <Icon name="fileText" />
          <strong>Loading degrees…</strong>
        </div>
      ) : filtered.length === 0 ? (
        <div className="amx-empty">
          <Icon name="fileText" />
          <strong>No degrees match your filters</strong>
        ...
        <div className="amx-table-wrap">
          <table className="amx-table">
            <thead>
              <tr>
                <SortHeader label="Degree / Qualification" sortKey="name" activeKey={sortKey} direction={sortDir} onSort={toggleSort} />
