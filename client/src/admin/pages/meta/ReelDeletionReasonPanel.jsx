...
const SORT_COLUMNS = {
  ...
  usageCount: { label: "Deletions Using This", get: (r) => r.usageCount || 0 },
...
function ReasonForm({ reason, onCancel, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = isEdit
        ? await adminApi.patch(`/reel-deletion-reasons/${reason.id}`, { name: name.trim(), isActive })
        : await adminApi.post("/reel-deletion-reasons", { name: name.trim(), isActive });
  ...
  return (
    <>
      <button className="amx-back-link" onClick={onCancel}>
        <Icon name="arrowRight" size={14} style={{ transform: "rotate(180deg)" }} /> Back to Reel Deletion Reasons
      </button>
      <h3 style={{ marginBottom: 20 }}>{isEdit ? "Edit Deletion Reason" : "Add Deletion Reason"}</h3>
      <form onSubmit={submit} style={{ maxWidth: 480 }}>
        <div className="amx-form-group">
          <label htmlFor="reel-deletion-reason-name">Reason</label>
          <input id="reel-deletion-reason-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Wrong video" autoFocus maxLength={255} />
          {error && (
    ...
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>
          {inUse
            ? `${reason.usageCount} past deletion${reason.usageCount === 1 ? "" : "s"} already used this reason and will keep it on record, but it will no longer be offered when a user deletes a Reel.`
            : "This reason will no longer be offered when a user deletes a Reel."}
        </p>
  ...
  return (
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>This can't be undone. Past deletions already recorded with this reason keep their record either way.</p>
        <div style={{ display: "flex", gap: 10 }}>
          <button className="amx-btn amx-btn-outline" style={{ flex: 1 }} onClick={onCancel} disabled={busy}>Cancel</button>
...
function ReelDeletionReasonPanel() {
  ...
  const load = () => {
    ...
    setError("");
    adminApi
      .get("/reel-deletion-reasons")
      .then(({ data }) => setReasons(data.reasons))
      .catch((err) => setError(err.response?.data?.message || "Couldn't load deletion reasons."))
  ...
  const activate = async (reason) => {
    ...
    try {
      await adminApi.patch(`/reel-deletion-reasons/${reason.id}`, { isActive: true });
  ...
  const confirmDeactivate = async () => {
    ...
    setBusyId(reason.id);
    try {
      await adminApi.patch(`/reel-deletion-reasons/${reason.id}`, { isActive: false });
  ...
  const confirmDelete = async () => {
    ...
    setBusyId(reason.id);
    try {
      await adminApi.delete(`/reel-deletion-reasons/${reason.id}`);
  ...
  return (
    <>
      <div className="amx-panel-head">
        <div>
          <h3>Reel Deletion Reasons</h3>
          <div className="amx-panel-sub">Master list of reasons offered when a user deletes their own Reel</div>
        </div>
      ...
      {loading ? (
        <div className="amx-empty">
          <Icon name="trash" />
          <strong>Loading deletion reasons…</strong>
        </div>
      ) : filtered.length === 0 ? (
        <div className="amx-empty">
          <Icon name="trash" />
          ...
          <table className="amx-table">
            <thead>
              <tr>
                ...
                <SortHeader label="Deletions Using This" sortKey="usageCount" activeKey={sortKey} direction={sortDir} onSort={toggleSort} />
...
export default ReelDeletionReasonPanel;
