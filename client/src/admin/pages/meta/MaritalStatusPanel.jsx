...
const SORT_COLUMNS = {
  name: { label: "Marital Status", get: (s) => s.name?.toLowerCase() || "" },
...
function StatusForm({ status, onCancel, onSaved }) {
  const isEdit = !!status;
  const [name, setName] = useState(status?.name || "");
  const [isActive, setIsActive] = useState(status ? status.isActive : true);
  ...
  const submit = async (e) => {
    ...
    try {
      const { data } = isEdit
        ? await adminApi.patch(`/marital-statuses/${status.id}`, { name: name.trim(), isActive })
        : await adminApi.post("/marital-statuses", { name: name.trim(), isActive });
      onSaved(data.status, isEdit);
    } catch (err) {
      setError(err.response?.data?.message || "Couldn't save this marital status.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <>
      <button className="amx-back-link" onClick={onCancel}>
        <Icon name="arrowRight" size={14} style={{ transform: "rotate(180deg)" }} /> Back to Marital Status
      </button>
      <h3 style={{ marginBottom: 20 }}>{isEdit ? "Edit Marital Status" : "Add Marital Status"}</h3>
      <form onSubmit={submit} style={{ maxWidth: 480 }}>
        <div className="amx-form-group">
          <label htmlFor="ms-name">Marital Status Name</label>
          <input id="ms-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Widowed" autoFocus maxLength={255} />
          {error && (
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
          <button type="submit" className="amx-btn amx-btn-primary" disabled={saving || !name.trim()}>
            {saving ? "Saving…" : isEdit ? "Save Changes" : "Add Marital Status"}
          </button>
...
function ConfirmDeactivateModal({ status, onCancel, onConfirm, busy }) {
  const inUse = (status.usageCount || 0) > 0;
  return (
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <h3 style={{ textAlign: "center" }}>Deactivate "{status.name}"?</h3>
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>
          {inUse
            ? `${status.usageCount} member${status.usageCount === 1 ? "" : "s"} already have this marital status and will keep it, but it will no longer be offered in the profile picker.`
            : "This marital status will no longer be offered in the profile picker."}
        </p>
...
function ConfirmDeleteModal({ status, onCancel, onConfirm, busy }) {
  return (
    <div className="amx-modal-overlay" onClick={onCancel}>
      <div className="amx-modal" onClick={(e) => e.stopPropagation()}>
        <button className="amx-modal-close" onClick={onCancel} aria-label="Close"><Icon name="x" size={16} /></button>
        <div className="amx-modal-danger-icon"><Icon name="trash" size={22} /></div>
        <h3 style={{ textAlign: "center" }}>Delete "{status.name}"?</h3>
        <p className="amx-modal-sub" style={{ textAlign: "center" }}>This can't be undone. Members who already have this marital status keep it either way.</p>
        <div style={{ display: "flex", gap: 10 }}>
          <button className="amx-btn amx-btn-outline" style={{ flex: 1 }} onClick={onCancel} disabled={busy}>Cancel</button>
...
function MaritalStatusPanel() {
  ...
  const [formModal, setFormModal] = useState(null); // null | "new" | status object (edit)
  ...
  const load = () => {
    ...
    adminApi
      ...
      .catch((err) => setError(err.response?.data?.message || "Couldn't load marital statuses."))
  ...
  const upsertStatus = (status, isEdit) => {
    setStatuses((ss) => (isEdit ? ss.map((s) => (s.id === status.id ? status : s)) : [...ss, status]));
    setFormModal(null);
    showToast(isEdit ? "Marital status updated." : "Marital status added.");
  };

  const activate = async (status) => {
    setBusyId(status.id);
    const prev = status.isActive;
    setStatuses((ss) => ss.map((s) => (s.id === status.id ? { ...s, isActive: true } : s)));
    try {
      await adminApi.patch(`/marital-statuses/${status.id}`, { isActive: true });
      showToast("Marital status activated.");
    } catch (err) {
      setStatuses((ss) => ss.map((s) => (s.id === status.id ? { ...s, isActive: prev } : s)));
      showToast(err.response?.data?.message || "Couldn't activate this marital status.");
    } finally {
      setBusyId(null);
    }
  };
  ...
  const confirmDeactivate = async () => {
    ...
    setBusyId(status.id);
    try {
      await adminApi.patch(`/marital-statuses/${status.id}`, { isActive: false });
      setStatuses((ss) => ss.map((s) => (s.id === status.id ? { ...s, isActive: false } : s)));
      showToast("Marital status deactivated.");
    } catch (err) {
      showToast(err.response?.data?.message || "Couldn't deactivate this marital status.");
    } finally {
      setBusyId(null);
  ...
  const confirmDelete = async () => {
    ...
    setBusyId(status.id);
    try {
      await adminApi.delete(`/marital-statuses/${status.id}`);
      setStatuses((ss) => ss.filter((s) => s.id !== status.id));
      showToast("Marital status deleted.");
    } catch (err) {
      showToast(err.response?.data?.message || "Couldn't delete this marital status.");
    } finally {
      setBusyId(null);
      setDeleting(null);
    }
  ...
  if (formModal) {
    return <StatusForm status={formModal === "new" ? null : formModal} onCancel={() => setFormModal(null)} onSaved={upsertStatus} />;
  }
    ...
    <>
      <div className="amx-panel-head">
        <div>
          ...
          <div className="amx-panel-sub">Master list of marital status options offered when members build their profile</div>
        </div>
        <button className="amx-btn amx-btn-primary" onClick={() => setFormModal("new")}>
          <Icon name="plus" size={15} /> Add Marital Status
        </button>
      </div>
      ...
      <div className="amx-filters">
        <div className="amx-search">
          <Icon name="search" />
          <input type="text" placeholder="Search by marital status…" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
      ...
      {loading ? (
        <div className="amx-empty">
          <Icon name="heart" />
          <strong>Loading marital statuses…</strong>
        </div>
      ) : filtered.length === 0 ? (
        <div className="amx-empty">
          <Icon name="heart" />
          <strong>No marital statuses match your filters</strong>
        ...
        <div className="amx-table-wrap">
          <table className="amx-table">
            <thead>
              <tr>
                <SortHeader label="Marital Status" sortKey="name" activeKey={sortKey} direction={sortDir} onSort={toggleSort} />
...
export default MaritalStatusPanel;
