...
function HobbyForm({ hobby, onCancel, onSaved }) {
  const isEdit = !!hobby;
  ...
  return (
    <>
      <button className="amx-back-link" onClick={onCancel}>
        <Icon name="arrowRight" size={14} style={{ transform: "rotate(180deg)" }} /> Back to Hobbies
      </button>
      <h3 style={{ marginBottom: 20 }}>{isEdit ? "Edit Hobby" : "Add Hobby"}</h3>
...
function HobbyPanel() {
  ...
  if (formModal) {
    return <HobbyForm hobby={formModal === "new" ? null : formModal} onCancel={() => setFormModal(null)} onSaved={upsertHobby} />;
  }
