...
const ENTITY_TYPE = "reel-deletion-reason";

async function usageCountsByName() {
  const rows = await CommunityActivity.findAll({
...
export const list = async (req, res) => {
  try {
    const reasons = await ReelDeletionReason.findAll({ order: [["sortOrder", "ASC"]] });
...
export const create = async (req, res) => {
  try {
    ...
    const maxOrder = (await ReelDeletionReason.max("sortOrder")) || 0;
    const reason = await ReelDeletionReason.create({
      name: name.trim(),
...
export const update = async (req, res) => {
  try {
    const reason = await ReelDeletionReason.findByPk(req.params.id);
