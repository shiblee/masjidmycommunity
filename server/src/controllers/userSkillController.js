...
export const create = async (req, res) => {
  try {
    const { skillId, customName, proficiency } = req.body;
    if (!skillId && !customName?.trim()) return res.status(400).json({ message: "Choose a skill or enter a custom one." });
    ...
    const entry = await UserSkill.create({
      ...
      customName: skillId ? null : customName.trim(),
