...
const Comment = sequelize.define(
  "Comment",
  {
    ...
    // Set only for a GIF/sticker comment (picked from the composer's
    // search) — body can still be non-empty alongside it (text + media in
    // the same comment). Nullable/no default so existing rows are untouched.
    mediaUrl: { type: DataTypes.STRING, allowNull: true },
    mediaType: { type: DataTypes.ENUM("gif", "sticker"), allowNull: true },
